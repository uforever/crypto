use crate::aes::{key_schedule, BLOCK_SIZE, S_BOX_FORWARD};
use crate::bytes::Bytes;
use crate::mode::Mode;
use crate::operation::Operation;
use crate::padding::Padding;
use crate::types::Result;

const ROTATE_FORWARD: [usize; 16] = [0, 5, 10, 15, 4, 9, 14, 3, 8, 13, 2, 7, 12, 1, 6, 11];

const MULT_2: [u8; 256] = [
    ,
];
const MULT_3: [u8; 256] = [
    ,
];

#[derive(Debug)]
pub struct AesEncrypt<M: Mode, P: Padding> {
    pub key: Bytes,
    pub mode: M,
    pub padding: P,
}

impl<M: Mode, P: Padding> AesEncrypt<M, P> {
    pub fn new(key: &[u8], mode: M) -> Self {
        Self {
            key: Bytes::new(key),
            mode,
            padding: P::build(BLOCK_SIZE),
        }
    }
}

impl<M: Mode, P: Padding> Operation for AesEncrypt<M, P> {
    fn run(&self, input: &[u8]) -> Result<Bytes> {
        let sub_keys = key_schedule(&self.key);
        let encrypt_func = block_encrypt(&sub_keys);
        let padded_data = self.padding.pad(input);

        Ok(self
            .mode
            .bytes_encrypt(&padded_data, BLOCK_SIZE, encrypt_func))
    }
}

fn block_encrypt(sub_keys: &[Bytes]) -> impl Fn(&[u8]) -> Bytes + '_ {
    move |block| {
        let mut result = Bytes::new(block);

        let rounds = sub_keys.len() - 1;
        result = result.xor(&sub_keys[0]);

        // 1..rounds
        for sub_key in sub_keys.iter().take(rounds).skip(1) {
            // substitution
            let subsituted: Vec<u8> = result
                .iter()
                .map(|byte| S_BOX_FORWARD[*byte as usize])
                .collect();
            // rotate
            let rotated = Bytes::new(subsituted).permutation(&ROTATE_FORWARD);

            // mix column
            let mut mixed = Vec::with_capacity(BLOCK_SIZE.into());
            for row in rotated.chunks(4) {
                mixed.push(MULT_2[row[0] as usize] ^ MULT_3[row[1] as usize] ^ row[2] ^ row[3]);
                mixed.push(row[0] ^ MULT_2[row[1] as usize] ^ MULT_3[row[2] as usize] ^ row[3]);
                mixed.push(row[0] ^ row[1] ^ MULT_2[row[2] as usize] ^ MULT_3[row[3] as usize]);
                mixed.push(MULT_3[row[0] as usize] ^ row[1] ^ row[2] ^ MULT_2[row[3] as usize]);
            }

            // add round key
            result = Bytes::new(mixed).xor(sub_key);
        }

        // substitution
        let subsituted: Vec<u8> = result
            .iter()
            .map(|byte| S_BOX_FORWARD[*byte as usize])
            .collect();
        // rotate
        let rotated = Bytes::new(subsituted).permutation(&ROTATE_FORWARD);
        rotated.xor(&sub_keys[rounds])
    }
}
